...
export const SCHEMA = [
  ...
  // 成熟度门槛(P2.3.3): 把扩散长度压回触角尺度之后(dw 0.06→0.02), 验收 ⑤d 从 108.2% 掉到 73.4%——
  // 记忆组在集体通道还在的建成期反而帮倒忙。根因不是记忆机制变了, 而是份额公式
  // rel = K_mem*forget*onRoute 里的 conf 是绝对浓度: 场一变薄, 蚂蚁闻不到路的时长就变长,
  // 记忆于是自动继承更大的转向份额——一条从没被走通验证过的线, 一提交就拿满权威。
  // 真实蚁的权威是逐趟验证出来的(ANT_BIOLOGY §二: 经验者走熟路、新手靠群体; L. niger 沿线保真
  // 87.4% 来自反复走同一根线), 所以补一个门: 同一根线连续被走通 K_route 次才拿满份额, 没走够就按比例给。
  // 判据用比例(memIA ≥ memNA*0.6)而不是绝对剂量 ⇒ 换场尺度不必重标, 这正是 HANDOVER §7 第二条要的尺度不变。
  // 出厂取 0 = 不门控(IEEE754 里 x*1 精确, 与 P2.3.2 逐位相同); 扫参选出真值后再单独一步改这个自由度。
  { key: 'K_route',    default: 0,   min: 0,  max: 6,   step: 1,    desc: '路线成熟度门槛: 同一根线被连续走通几次才拿满记忆份额(0=不门控, 与 P2.3.2 逐位相同)' },
