...
export const SCHEMA = [
  ...
  // ---- P2.3.4 光污染 III：侧抑制背景减除（纯渲染层，出厂=0 即逐字节不变） ----
  // 病根换了：残留的雾不是「太亮」而是**空间上的共模**——缓变的一大片，蚂蚁从里面读不出方向，
  // 却和走廊一样占亮度预算。实测靠曝光压它：乘数 ×1→×4 死光只 5.4%→4.4%，走廊级数却 85→25，
  // 越压越糊 ⇒ 曝光这条路已到天花板，得换「减局部平均」这条路。
  // 生物学依据：①触角叶突触前侧抑制做增益控制(Bahrey & Wilson 2004, Nature 452:246)——邻居都在响
  //   的共模被压掉，只有相对差往下传；②昆虫 AL 用 lateral inhibition 做 contrast enhancement，
  //   感受野 center-surround(Galizia & Szyszka, J Comp Physiol A)；③弓背蚁循迹靠两根触角检测
  //   **轨迹边缘**来决定转向(Draft et al. 2018, J Exp Biol 221)——所以这张图改显示「反差」而不是「分子数」。
  ...
  { key: 'lateralK',      default: 0,    min: 0,    max: 2,   step: 0.01,  desc: '侧抑制: 显示量=浓度−「触角半径那一圈」的均值×该值(0=关,画面逐字节不变 / 1=纯共模抑制,只留反差)' },
