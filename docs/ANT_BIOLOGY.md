...
| 14 | 显示量 = 浓度 −「触角半径那一圈」的均值（`lateralK=0.5`，**纯渲染层**） | 昆虫触角叶用**突触前侧抑制**做增益控制（Olsen & Wilson 2008），同一套抑制带来**浓度不变性**（Kim & Wang 2009）；弓背蚁循迹靠两根触角**跨轨迹边缘**采样决定转向（Draft et al. 2018） | ⚠️ **真实对应发生在蚂蚁的触角叶，我们发生在屏幕上**＝给观众的眼睛装一个外置触角叶。蚂蚁那一侧的反差已由第 1 行（Weber 律三触角）在 sim 里承担，本机制**不改 sim 一个位** ⇒ 它不算"蚂蚁这样"，算"看图该这样"（详见 §二 末） |
...
**侧抑制、增益控制与"这张图是给谁看的"（P2.3.4）**：真实昆虫的第一级嗅觉中枢（触角叶）不是化学计量仪。
局域中间神经元**压在嗅觉传入神经末梢上**的突触前侧抑制做**增益控制**——"邻居们都在响"那份共模被压掉，
只有相对差能往下传（Olsen & Wilson 2008）；同一套抑制带来**浓度不变性**，同一团气味在不同绝对浓度下被
认成同一个东西（Kim & Wang 2009）。行为层面，弓背蚁循迹时两根触角反复**跨轨迹边缘**采样，用左右之差
决定下一步转向（Draft et al. 2018）。⇒ 蚂蚁算的是"差"，不是"量"。

本项目 P2.3.4 把这条原理用在**屏幕**上：显示量 = 浓度 −「触角半径那一圈」的均值 × `lateralK`（出厂 0.5）。
**这里必须写清一处不对称**：真实对应发生在蚂蚁的触角叶，我们发生在观众的眼睛前——这是给人眼装一个外置
触角叶，不是声称蚂蚁的视野里长着蓝雾。蚂蚁那一侧的反差计算早就由对照总表第 1 行（Weber 律三触角）在
sim 里承担了，`lateralK` **不改 sim 一个位**（关掉它，画面逐字节退回上一版，见 METRICS P2.3.4 §8）。
⇒ 凡是"只改显示"的机制，都要回这里回答同一个问题：**它替谁做了这个生物学动作**。答不上来的标"工程需要"。
...
- [Olsen & Wilson 2008, Nature 452(7190):956–960 — 触角叶突触前侧抑制做增益控制【PMID 18344978】](https://pubmed.ncbi.nlm.nih.gov/18344978/)
- [Kim & Wang 2009, J Biol 8(1):4 — 侧抑制与浓度不变性【PMID 19216732】](https://pubmed.ncbi.nlm.nih.gov/19216732/)
- [Draft, McGill, Kapoor & Murthy 2018, J Exp Biol 221:jeb185124 — 弓背蚁触角跨轨迹边缘采样【PMID 30266788】](https://pubmed.ncbi.nlm.nih.gov/30266788/)
  - 这三条 2026-09-05 逐条对 NCBI 核到作者/卷期页/PMID。**上一轮写进代码注释的两条引用是凭印象编的**
    （"Bahrey & Wilson 2004"、"Draft, Burd-Field, Kain & Graham"），作废过程见 METRICS P2.3.4 §2。
