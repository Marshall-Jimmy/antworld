...
export class Panel {
  constructor(params) {
    ...
    const groupOf = (key) => {
      if (['foodLoadRate','carryTimeout','forageTimeout','nestRadius','speed','leak','K_mem','K_route','memStep','memForget'].includes(key)) return '运动 / 记忆';
      ...
      if (['diffuseWeight','decayRate','peak','toneMap','autoPeak','lateralK','alarmDecay','alarmSplash','alarmPeak','emptyDeposit'].includes(key)) return '场';
      return '世界';
    };
